...
async function consultarEndereco() {
    ...
    if (!/^\d{8}$/.test(cep) || !cidadeInformada.trim()) {
        resultado.textContent = "Dados inválidos. Volte e informe CEP e cidade.";
        return;
    }
    ...
    try {
        ...
        if (normalizar(cidadeInformada) !== normalizar(dados.localidade)) {
        ...
        linkMaps.href = `https://www.google.com/maps/search/?api=1&query=${busca}`;
...
// Executa a consulta ao carregar a página
consultarEndereco();